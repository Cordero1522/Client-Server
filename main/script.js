(function(){
    ...
    // ⚠️ REEMPLAZAR CON TU URL DE GOOGLE APPS SCRIPT DE ENVIO DE DATOS
    const GOOGLE_SCRIPT_URL = 'https://script.google.com/macros/s/AKfycbwFAJ2TE4pd6CtUdBAxox535-1FNLCtz2ZDTHxPwLlSJ3V8OdWnHFtAKapDDaBYXbABOA/exec';
    // ⚠️ REEMPLAZAR CON TU URL DE GOOGLE APPS SCRIPT DE SUSCRIPCIÓN
    ...
    function renderCatalogo() {
        const productosFiltrados = categoriaActual === 'todos' 
            ? catalogo 
            : catalogo.filter(prod => prod.categoria === categoriaActual);
        
        productosContainer.innerHTML = '';
        
        if (productosFiltrados.length === 0) {
            productosContainer.innerHTML = '<p style="text-align:center; grid-column:1/-1; padding:60px;">No hay productos en esta categoría.</p>';
            return;
        }
        ...
        productosFiltrados.forEach(prod => {
            ...
            card.innerHTML = `
                <img class="producto-imagen" src="${prod.img}" alt="${prod.nombre}" loading="lazy">
                <div class="product-info">
                    <div class="producto-nombre">${prod.nombre}</div>
                    <div class="producto-categoria">${getCategoriaNombre(prod.categoria)}</div>
                    <div class="product-desc">${prod.desc}</div>
                    <div class="producto-precio">$${prod.precio.toLocaleString('es-MX')}</div>
    ...
    carrito.forEach(item => {
        // Buscar el producto completo para obtener su imagen
            ...
            <div class="cart-item">
                <div style="display: flex; gap: 15px; align-items: center; flex: 1;">
                    ...
                    <div class="cart-item-info">
                        <h4>${item.nombre}</h4>
                        <span class="cart-item-precio">$${item.precio.toLocaleString('es-AR')} c/u</span>
                    </div>
                </div>
    ...
    async function enviarPedidoASheets() {
        if (!GOOGLE_SCRIPT_URL || GOOGLE_SCRIPT_URL.includes('YOUR_SCRIPT_ID')) {
            mensajeDiv.innerHTML = '<span style="color:#b34a3c;">❌ Configura la URL de Google Apps Script.</span>';
            return;
        }

        if (carrito.length === 0) {
            mensajeDiv.innerHTML = '<span style="color:#b34a3c;">⚠️ El carrito está vacío.</span>';
            return;
        }
        ...
        mensajeDiv.innerHTML = '<span style="color:#1f4f3d;">⏳ Procesando pedido...</span>';
    ...
    if (!email) {
        mostrarMensajeNewsletter('❌ Ingresa un correo válido.', 'error');
        return;
    }
    
    if (!email.includes('@') || !email.includes('.')) {
        mostrarMensajeNewsletter('❌ Correo inválido.', 'error');
        return;
    }
    ...
    try {
        ...
        mostrarMensajeNewsletter('✅ ¡Suscripción exitosa!', 'exito');
        emailInput.value = '';
        
    } catch (error) {
        mostrarMensajeNewsletter('❌ Error al suscribir.', 'error');
    } finally {
...
function mostrarMensajeNewsletter(mensaje) {
    let msgDiv = document.getElementById('newsletterMsg');
    if (!msgDiv) {
        msgDiv = document.createElement('div');
        msgDiv.id = 'newsletterMsg';
        msgDiv.style.cssText = 'font-size:0.8rem;margin-top:8px;color:white';
        document.querySelector('.footer-social').appendChild(msgDiv);
    }
...
if (suscribirBtn) {
    ...
    // Scroll suave SOLO para el botón "Ver catálogo" y el link del footer
const botonesCatalogo = document.querySelectorAll('.btn-primary[href="#catalogo"], .footer-links a[href="#catalogo"]');
